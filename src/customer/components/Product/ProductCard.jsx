...
const ProductCard = ({ Eachproducts }) => {
  const { title, brand, imageUrl, price, discountedPrice, color, discountPersent } = Eachproducts;

  const navigate = useNavigate();

  const handleNavigate = () => {
    navigate(`/product/${Eachproducts.id}`)
  }
  return (
    <div onClick={handleNavigate} className="productCard w-[15rem] m-3 transition-all cursor-pointer">
      <div className="h-[20rem] ">
        {console.log(Eachproducts+"one product")}
